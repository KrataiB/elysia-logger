...
export class Logger {
  ...
  private static cachedTime: string = new Date()
    .toISOString()
    .replace("T", " ")
    .substring(0, 19);
  ...
  private formatMessage(
    ...
    return `${colors.green}[${this.options.name || "Elysia"}]${colors.reset} ${
      colors.dim
    }${pid}  -${colors.reset} ${colors.dim}${Logger.cachedTime}${
      colors.reset
    }   ${levelColor}${level.toUpperCase()}${colors.reset} ${
      colors.yellow
    }[${ctx}]${colors.reset} ${message}\n`;
  }
